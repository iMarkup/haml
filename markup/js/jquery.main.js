...
// responsive gallery init
function initResponsiveGallery() {
	jQuery('.cycle-gallery').multiSlideCyclicGalery({
		mask: '.mask',
		slideset: '.slideset',
		slides: '.slide',
		btnPrev: '.btn-prev',
...
;(function($) {
	...
	MultiSlideCyclicGalery.prototype = {
		...
		findElements: function() {
			...
			this.currentNumber = this.gallery.find(this.options.currentNumber);
			this.totalNumber = this.gallery.find(this.options.totalNumber);
			...
			if (this.totalNumber.length) {
				this.totalNumber.text(this.slides.length);
			}
		},
		attachEvents: function() {
			var self = this;
		...
		resizeSlides: function() {
			var sidesOffset = this.slides.eq(0).outerWidth(true) - this.slides.eq(0).outerWidth();

			this.slides.css({
				width: this.mask.outerWidth() / this.options.visibleSlides - sidesOffset
			});
		},
		refrestCounter: function() {
			var leftOffset = Infinity;
			var num = 0;
			...
			if (this.currentNumber.length) {
				this.currentNumber.text(num);
			}
		},
		sliderWidth: function() {
			this.slideset.css({
				width: this.getStepOffset()
			});
		},
		...
		getAppendedSlides: function() {
			var slides = this.slideset.find(this.options.slides);
			var currSlides = $();
			var self = this;
			...
			slides.each(function(i) {
				...
				if (self.direction > 0) {
					if (i < self.options.slidesToScroll) {
						currSlides = currSlides.add(slide);
					}
				} else {
					if (i >= slidesLength - self.options.slidesToScroll) {
						currSlides = currSlides.add(slide);
					}
				}
			});
		...
		switchSlide: function() {
			var self = this;
			var appendedSlides = this.getAppendedSlides();
			var animDistance = this.mask.outerWidth() / this.options.visibleSlides * this.options.slidesToScroll;

			if (this.direction > 0) {
				this.slideset.animate({
					marginLeft: -animDistance
				}, {
					duration: this.options.animSpeed,
					complete: function() {
						appendedSlides.appendTo(self.slideset);
						self.slideset.css({
							marginLeft: ''
						});
						self.refrestCounter();
				...
				appendedSlides.prependTo(this.slideset);
				this.slideset.css({
					marginLeft: -animDistance
				});
				this.slideset.animate({
